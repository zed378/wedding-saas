...
- Canonical URL using the invitation's active address — `https://invitation.vizunicum.my.id/{slug}` at MVP (PLAN/10). If the address format changes later, the canonical changes with it and the old form redirects permanently, so link equity and forwarded WhatsApp links both survive.
