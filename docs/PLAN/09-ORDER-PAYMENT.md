...
- As implemented (P3-08, ADR-079): generated by a job after the payment commits, or on the first download if that job has not run; stored once and never re-rendered. A trial invitation's first payment is labelled a publication, not a renewal. Who the invoice names as seller is `OQ-29`.
