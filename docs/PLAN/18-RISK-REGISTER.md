...
| R16 | Gift account tampering: an attacker who changes `account_number` on a published invitation redirects every guest's gift to themselves, and the couple discovers it only after the wedding | Security/Financial | Very High | Low | Object-level authorization on every write (SECURITY/05), audit trail of bank account changes, and a non-optional email to the owner whenever gift details change on a published invitation | MEMORY ADR-025, SECURITY/09, PLAN/13 |
