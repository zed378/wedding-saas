...
|---|---|---|---|
| 2026-09-11 | P0-23 (partial) | [Staging deployed](./records/2026-09-11-P0-23-staging-deploy.md) | Ten services running on a real host, migrated and seeded. Deploying forced `APP_ENV` apart from `NODE_ENV`: staging must run a production **build** while being a non-production **environment**, and keyed on `NODE_ENV` a **live payment key on staging would have passed**. Hostnames still blocked on one Cloudflare permission |
...
- `P0-23` — [Staging deployed](./records/2026-09-11-P0-23-staging-deploy.md) — partial, and written up before completion because the deployment found two bugs and forced two decisions
