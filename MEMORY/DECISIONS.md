...
### ADR-079 — Invoices: a dependency-free PDF, stored once in the database, generated after payment

**Date** 2026-09-15 · **Task** `P3-08` · **Status** Accepted · **Deviates from** ADR-017 (PDF library) · **Amends** `docs/API/06`, `docs/DATABASE/07`, `docs/PLAN/09` · **Raises** `OQ-29`

**Context** — `P3-08` needs a downloadable invoice after payment, generated outside the request. ADR-017
chose `@react-pdf/renderer`; it is not installed, and would bring React and a layout engine into the API
for a dozen lines of text. Object storage has no private bucket for documents, and the card requires that
a generation failure cannot affect payment state. No document names the seller.

**Decisions**

1. **A small PDF 1.4 writer** (`simple-pdf.ts`): one A4 page, built-in Helvetica/Helvetica-Bold, text and
   rules at fixed positions, an exact cross-reference table (tested offset by offset). Characters outside
   WinAnsi become `?` — stated limit; an invoice needing other scripts needs an embedded font and a library.
2. **Stored in a new `invoices` table** (migration `0014`), inserted once, INSERT/SELECT only for the
   application role. Not object storage: private by construction, no bucket to add.
3. **Generated by `invoice.generate`**, queued by the webhook after it commits and consumed by the API jobs
   process (ADR-078); the owner's first download generates it if the job has not run. Both go through
   `InvoiceService.generate`, so the document is the same either way.
4. **One line at `orders.amount_total`**: per-item prices at purchase are not stored, and current catalogue
   prices might not add up to what was paid.
5. **"Publikasi" vs "Perpanjangan"** from whether an earlier order for the invitation was paid, not from
   `order_type` — closing the `P3-02` note that a trial's first payment is typed `renewal`.
6. **Seller identity is configuration** (`INVOICE_SELLER_NAME`, default the domain; `INVOICE_SELLER_ADDRESS`),
   and the document makes no legal or tax claim, pending `OQ-29`.
