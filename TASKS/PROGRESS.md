...
**Current phase**: Phase 1 — Auth and Invitation Core (24 / 25 done). Phase 0 is 26 / 27 done. **Staging is live**: `https://app.vizunicum.my.id` and `https://invitation.vizunicum.my.id/{slug}`, served from the VM at `10.1.200.13` through a Cloudflare Tunnel — the host has a private address and no inbound port.
...
| [Phase 1 — Auth and Invitation Core](./PHASE-1-AUTH-AND-INVITATION-CORE.md) | 25 | 24 | **ACTIVE** | Phase 0 exit criteria |
