...
**Current phase**: Phase 3 — Order, Payment and Publishing (9 / 16 done). **Phase 2 is complete** (18 / 18 including the owner-requested `P2-16`, `P2-17` and `P2-18`; [summary](../MEMORY/records/2026-09-13-PHASE-2-SUMMARY.md)); **Phase 1 is complete** (25 / 25, [summary](../MEMORY/records/2026-09-12-PHASE-1-SUMMARY.md)); Phase 0 is 26 / 27 done. **Staging is live** but runs code from before Phase 2's fixes: redeploy, run migrations `0007`–`0014`, start the new `api-jobs` service, `db:seed:regions`, and reseed before showing it to anyone. `https://app.vizunicum.my.id` and `https://invitation.vizunicum.my.id/{slug}`, served from the VM at `10.1.200.13` through a Cloudflare Tunnel — the host has a private address and no inbound port.
...
| [Phase 3 — Order, Payment and Publishing](./PHASE-3-ORDER-PAYMENT-PUBLISHING.md) | 16 | 9 | **ACTIVE** | Phase 2 exit — **met** (LCP qualified by `OQ-26`) | Phase 2 exit — **met** (LCP qualified by `OQ-26`) |
...
| P1-13 | Bank accounts and quote | backend | M | **DONE** — the threat is substitution, not disclosure (R16): every change is audited inside its own transaction, and a change to a published invitation notifies the owner | P1-10 |
| P1-14 | Settings and slug rules | backend | M | **DONE** — the boundary rejects rather than silently ignoring; theme override **values** are validated too, because they become CSS custom properties on a public page | P1-10 |
| P1-15 | Change template without data loss | backend | M | **DONE** — nothing is deleted, proven three ways: row counts, an A→B→A round trip, and a source check that the path contains no `delete` | P1-10, P0-20 |
...
| P1-24 | Frontend — media manager and map picker | web-app | L | **DONE** — keyboard reorder is the only mechanism, not a fallback; the map degrades to two coordinate inputs because MapLibre needs WebGL. Drag and the delete-undo are **not** built, and are named as deviations | P1-23, P1-19 |
| P1-25 | Phase 1 test suite and acceptance | all | L | **DONE** — 24 endpoints swept over HTTP against the real app, four cases each, matrix generated by the run and clean. The first mutation proved the sweep could not see the bug it was aimed at: the parent constraint is only observable through the caller's OWN second invitation | all above |
...
| P2-03 | Section component library v1 | frontend | L | **DONE** — every visual value from the theme, now a build failure otherwise. Visual snapshots deferred to P2-13, which already needs a browser | P2-02 |
| P2-04 | Per-section error boundaries | frontend | M | **DONE** — all three levels plus the editor preview. Reports carry no section data, because the props behind a render error are sometimes account numbers. Nothing forwards them to Sentry yet — a seam, named | P2-03 |
| P2-05 | Live preview in the editor | web-app | M | **DONE** — and DF-11 found a real bug: the editor's three-column desktop layout never worked, because Tailwind v4's preflight gives `[hidden]` `display:none!important`. Also closed PG-19 (ADR-060): the editor could not reach its locked template version | P2-03, P1-22 |
| P2-06 | Publish-check and editor checklist | backend, web-app | M | **DONE** — one resolver serves the endpoint, the editor button and the change-template guard, so client and server cannot disagree about what is complete. Answered OQ-23 (ADR-061): a published invitation may change template, but the required-field check re-runs first | P0-20, P1-14 |
...
| P2-16 | Event timezone (WIB/WITA/WIT) | all | M | **DONE** — added after Phase 2 closed, from the owner's answer to `OQ-27`: the zone is detected from the map pin and can be chosen in the form. Every event was previously labelled and counted down as WIB (ADR-070) | P2-15 |
| P2-17 | Indonesian administrative regions (province to village) | backend, web-app | L | **DONE** — at the owner's request: all 38 provinces, 514 regencies/cities, 7,285 districts and 83,762 villages (Kepmendagri 300.2.2-2138/2025 via `cahyadsn/wilayah`, MIT), with boundaries. Public cached API; an event's region picker; the region's province now decides the zone exactly, and a pin is checked against real province boundaries (ADR-071) | P2-16 |
| P2-18 | Event date format: 15 Mei 2027 | schema, renderer, public-invite, web-app | S | **DONE** — the owner's answer to `OQ-27`'s format half; one table-based formatter replaces raw ISO dates everywhere guests and couples read them (ADR-072) | P2-16 |
...
| P3-03 | Payment gateway port and adapter | backend | L | **DONE** — port with no unverified event type; Midtrans Snap adapter from Midtrans's docs; fake adapter; production requires a server key; the unused webhook secret removed (ADR-075) | P3-02, P0-18 |
...
| P3-08 | Invoice and order history | backend | M | **DONE** — `GET /orders`, `/orders/:id`, `/orders/:id/invoice`; dependency-free PDF stored once in `invoices` (migration `0014`), generated by a job after payment or on first download; raised `OQ-29` (seller identity) (ADR-079) | P3-05 |
| P3-09 | Publish endpoint | backend | L | **DONE** — paid publish and BR-2.8 trial (new `draft → published` edge, history-checked); trial notice on the public page; E2E live < 5 s; **fixed: completeness checks read the editor DTO instead of the canonical document, so nothing on the reference template could ever be published** (ADR-080) | P2-06, P3-05 |
