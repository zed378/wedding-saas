...
| | |
|---|---|
| **Status** | **DONE** — 2026-09-12, [record](../MEMORY/records/2026-09-12-P1-25-phase-1-acceptance.md), [phase summary](../MEMORY/records/2026-09-12-PHASE-1-SUMMARY.md) |
...
**Definition of Done**
- [x] The IDOR matrix is complete with zero failures, and is committed as part of the phase record — [`2026-09-12-P1-25-idor-matrix.md`](../MEMORY/records/2026-09-12-P1-25-idor-matrix.md), 24 endpoints × 4 cases, **generated by the run** so it cannot claim a result the suite did not produce.
- [x] Mass-assignment and upload abuse suites pass — and both were mutation-checked, the sanitiser and the magic-byte check each caught by a named test when removed.
- [x] The autosave and template-switch acceptance criteria are verified with named tests — `"keeps all hundred of a hundred consecutive edits"`, `"editing 100 fields in sequence loses nothing (DoD 1)"`, `"hides fields on a template switch and gives every one of them back"`.
- [x] Service-layer coverage meets the 80% gate — **97.24% lines**, via `pnpm --filter @wi/api test:coverage`. It measures both suites in one run, because v8 coverage from two runs does not merge and the unit suite stubs its services on purpose.
- [x] The phase summary exists in `MEMORY/records/` with its deviations and deferrals named.

**Added beyond the steps** — `docs/TESTING/04` § XSS Test was in no card's step list, and
nothing in the repository had ever sent a payload through a **route**. Steps 1 and 2 are now
covered by `"stores every free-text field with its markup stripped"`; step 3 needs the public
renderer and belongs to `P2-03`.

**One deviation**, ADR-058: `POST /gallery/reorder` answers a foreign photo id 422 rather
than 404, because its body is a set rather than an id. The sweep asserts the stronger
property — byte-identical to the answer an id that never existed gets.
