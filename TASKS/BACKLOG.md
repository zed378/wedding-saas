...
| DF-11 | A desktop-viewport Playwright check of the editor's three-column layout | `P1-22`, reassigned by `P1-25` | **Blocked on `P2-01`, not on effort.** jsdom evaluates no media queries, so every editor component test asserts the **mobile** arrangement. A browser check needs the editor reached with a session — and the wizard renders `templates={[]}` until the catalogue API exists, so no user can create an invitation and no editor route can be opened. Moves to `P2-05`, which is the next card that touches the editor |
