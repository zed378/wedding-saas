...
| PG-18 | `P1-21`'s DoD requires slug availability to be checked **before submission**, and no endpoint existed to check it with — `docs/API/04` specified creation and settings, both of which discover a collision by failing | `GET /api/v1/invitations/slug-available`, advisory, authenticated and rate limited; the creation call stays authoritative | ADR-057 | `API/04` |
| PG-19 | The editor cannot reach the definition of the template version its invitation is locked to. `docs/API/04`'s detail response carries `template_id` and `template_version_id` as bare uuids; `docs/API/03` offers `GET /templates/:slug` (the **newest** published version, which BR-3.1 says is the wrong one for an existing invitation) and `GET /templates/:slug/versions/:version`, which needs a slug and a semver the client does not have. Without a resolution the live preview and the schema-driven properties panel have no section list to work from | The detail response gains `template: { slug, name }` and `template_version: { id, version }` — the two identifiers a client needs to call the documented catalogue endpoint. Not the whole definition: that would put a large JSON blob on every invitation read and duplicate `P2-01`'s cache | ADR-060 | `API/04` |
...
| ~~DF-11~~ | A desktop-viewport Playwright check of the editor's three-column layout | `P1-22`, reassigned by `P1-25`, **closed by `P2-05`** | **Done, and it found a real bug**: the layout never worked, because Tailwind v4's preflight gives `[hidden]` `display:none!important` and `md:block` cannot override it. Discharged on the workbench rather than at `/editor/:id`, because DF-11's question is a CSS one about the shell. Originally: jsdom evaluates no media queries, so every editor component test asserts the **mobile** arrangement. A browser check needs the editor reached with a session — and the wizard renders `templates={[]}` until the catalogue API exists, so no user can create an invitation and no editor route can be opened. Moves to `P2-05`, which is the next card that touches the editor |
...
| DF-12 | A "Most Popular" sort for the template catalogue | `docs/UI-UX/11` § Catalog Page ("based on usage count, **if data is available**") | Not available: the catalogue API is anonymous and has no usage count, and a count is a query over tenant invitations that must exclude the system account's demos. `P2-11` ships Newest only and says so on the page (ADR-065). Worth revisiting once there are enough templates for order to matter — likely with `P5`'s admin analytics, which will need the same count |
